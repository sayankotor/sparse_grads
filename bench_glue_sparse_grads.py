...
from exps.sparse_grad import replace_bert_layers
...
class TrainerBert(Trainer):
    ...
    def make_grad_bank(self, taskname, out_dir):
        self.n_show = 10
        self.losses = []
        ...
        for layer in range(12):
            ...
            if (len(model.bert.encoder.layer[layer].intermediate.dense.grads)) < 31:
                ...
                if (self.n_show > 0):
                    ...
                    plt.show()
                    plt.savefig(self.out_dir + '/grad_20x20 ' + str(self.n_show))
...
class ModelArguments:
    ...
    collect_grads: bool = field(
        default=True,
        metadata={"help": "create 30 matrices of args."},
    )
    exp_name: str = field(
        default='none',
...
def main(tasks_):
    ...
    if data_args.task_name is not None:
        ...
        raw_dataset_cut = load_dataset(
            "glue",
            data_args.task_name,
            cache_dir=model_args.cache_dir,
            use_auth_token=True if model_args.use_auth_token else None, split='train[:10%]')
    ...
    if model_args.collect_grads:
        if "train" not in raw_datasets:
            raise ValueError("--do_train requires a train dataset")
        train_dataset_cut = raw_dataset_cut
    ...
    trainer.make_grad_bank(data_args.task_name, training_args.output_dir)
    
    if model_args.collect_grads:
        #trainer.train_dataset = train_dataset_cut
