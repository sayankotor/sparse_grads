...
def sparse_to_dense_ein(input, weight, rows, cols):
    b, r, c = input.shape
    #rows, cols = torch.nonzero(input.reshape(b,r*c), as_tuple=True)
    #rows, cols = torch.unique(rows), torch.unique(cols)
    res_indirect = input.reshape(b,r*c)[rows, :][:, cols] @ weight.reshape(-1, weight.shape[2])[cols, :]

    # res_indirect = res_indirect.reshape(b,r, weight.T.shape[1])
    #print (res_indirect.shape)
    #I = torch.eye(r*c).to('cuda')
    #res = I[:, rows] @ res_indirect
    #res = res.reshape(b, weight.shape[2] )
...
class LinearFunctionSparseGrad(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if (layer_type == torch.tensor(0)):
            grad_output, rows, cols = sparse_to_dense(grad_output, SparseGradLinearIntermediate._VT)
            #grad_output = torch.sparse.smm(grad_output, SparseGradLinearIntermediate._VT)
        else:
            grad_output, rows, cols = sparse_to_dense(grad_output, SparseGradLinearOutput._VT)
        ...
        if ctx.needs_input_grad[0]:
           ...
           grad_input = grad_output@weight#sparse_precount_to_dense(grad_output, weight, rows, cols, b, r, c)
        
        if ctx.needs_input_grad[1]:
            #grad_weight = torch.einsum('ijk,kjl->il', grad_output.T, input)#grad_output.T @input
            grad_weight = sparse_to_dense_ein(grad_output.T, input, cols, rows)#sparse_precount_to_dense_T(grad_output.T, input, rows, cols)
