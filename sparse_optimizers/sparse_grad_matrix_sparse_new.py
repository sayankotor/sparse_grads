...
class LinearFunctionSparseGrad(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if (layer_type == torch.tensor(0)):
            grad_output, rows, cols, b, r, c = sparse_to_dense_prep(grad_output, SparseGradLinearIntermediate._VT)
        else:
            grad_output, rows, cols, b, r, c = sparse_to_dense_prep(grad_output, SparseGradLinearOutput._VT)
