...
def _full_rank_tt_fisher(
    ...
    device = data.device
    result = []
    print ("in ful rank tt fisher data.shape", data.shape)
    if batch:
        N = data.dim() - 1
    ...
    for n in range(1, N):
        if resh.shape[d1] < resh.shape[d2]:
            print ("resh.shape[d1] < resh.shape[d2]")                
            if batch:
                aux = torch.diag(torch.sum(resh, 2))#.repeat(resh.shape[0], 1, 1)
                result.append(aux.reshape([resh.shape[0], resh.shape[1] // shape[n], shape[n], resh.shape[1]]))
                a = resh.reshape([resh.shape[0], resh.shape[1] // shape[n], shape[n], resh.shape[1]])
                resh = torch.reshape(resh, (resh.shape[0], resh.shape[1] * shape[n + 1], resh.shape[2] // shape[n + 1]))
            else:
                aux = torch.diag(torch.sum(resh, 1))#torch.eye(resh.shape[0], device=device, dtype=dtype)
                print ("i shape 0", aux.shape)
                result.append(aux.reshape([resh.shape[0] // shape[n - 1], shape[n - 1], resh.shape[0]]).to(device))
                resh = torch.reshape(resh, (resh.shape[0] * shape[n], resh.shape[1] // shape[n]))
                print ("resh shape in", resh.shape)
        else:
            print ("else")
            if batch:
                aux = resh.reshape([resh.shape[0], resh.shape[1] // shape[n], shape[n], resh.shape[2]])
                result.append(aux)
                I = torch.eye(resh.shape[2], device=device, dtype=dtype).repeat(resh.shape[0], 1, 1)
                resh = aux.reshape((resh.shape[0], resh.shape[2] * shape[n + 1], resh.shape[2] // shape[n + 1]))
            else:
                aux = resh.reshape([resh.shape[0] // shape[n - 1], shape[n - 1], resh.shape[1]])
                if (resh.shape[1]*resh.shape[1] != np.prod(aux.shape)):
                    aux = torch.diag(torch.sum(resh, 0))  
                print ("aa.shape", aux.shape)
                result.append(aux)
                I = torch.eye(resh.shape[1], device=device, dtype=dtype)
                print ("I shape", I.shape)
                resh = aux.reshape(resh.shape[1] * shape[n], resh.shape[1] // shape[n]).to(device)
                print ("resh shape", resh.shape)
...
def _full_rank_tt(
    ...
    device = data.device
    result = []
    print ("in ful rank tt data.shape", data.shape)
    if batch:
        N = data.dim() - 1
        resh = data.reshape([*shape[:2], -1])
        d1 = 1
...
class CustomTensor(object):
            ...
            if ranks_cp is not None:  # Compute CP from full tensor: CP-ALS
                ...
                for iter in range(max_iter):
                    for n in range(self.dim()):
                        ...
                        self.cores[n] = torch.linalg.lstsq(prod, unf_khatri_t).solution.transpose(-1, -2)
                ...
                self.cores = _full_rank_tt(data, batch)
                self.fcores = _full_rank_tt_fisher(fdata, batch)
    ...
    def round_tt(
        ...
        for mu in range(N - 1, 0, -1):
            ...
            print ("after mu core shape", self.cores[mu].shape)
            print ("after mu-1 core shape", self.cores[mu-1].shape)
            print ("\n\n\n")
