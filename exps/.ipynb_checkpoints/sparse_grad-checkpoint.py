...
class LinearFunctionSparseGrad(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, bias, grads, treshold, U, VT):
        ...
        return  input @ VT.T @ weight.T @ U.T  + bias # space 2
...
class SparseGradLinear(torch.nn.Module):
    ...
    def create_UV(self):
        if (len(self.grads)>=30):
            u1, U, VT = Tucker_Decomposition(torch.stack(self.grads[:30]))
            self.U = U
            self.VT = VT
            #print ("self.U", self.U.shape, "self.VT", self.VT.shape)
            self.weight = torch.nn.Parameter(self.U@self.weight@self.VT)
        else:
